...
namespace CustomControls.StringComparison
{
    ...
    public static class DoubleMetaphone
    {
        public static string Encode(string text)
        {
            ...
            int last = text.Length - 1; //zero based index

            string workingString = text.ToUpperInvariant() + "     ";

            bool isSlavoGermanic = (text.IndexOf('W') > -1) || (text.IndexOf('K') > -1) || (text.IndexOf("CZ", System.StringComparison.OrdinalIgnoreCase) > -1) || (text.IndexOf("WITZ", System.StringComparison.OrdinalIgnoreCase) > -1);

            //skip these when at start of word
            if (workingString.StartsWith(System.StringComparison.OrdinalIgnoreCase, "GN", "KN", "PN", "WR", "PS"))
            {
                current += 1;
            }
                ...
                switch (workingString[current])
                {
                    ...
                    case 'C':
                        ...
                        if (StringAt(workingString, current, "CH"))
                        {
                            ...
                            if ((current == 0) && (StringAt(workingString, (current + 1), "HARAC", "HARIS") || StringAt(workingString, (current + 1), "HOR", "HYM", "HIA", "HEM")) && !StringAt(workingString, 0, "CHORE"))
                            {
                                metaphoneData.Add("K");
                                current += 2;
                                break;
                            ...
                            if ((StringAt(workingString, 0, "VAN ", "VON ") || StringAt(workingString, 0, "SCH")) // 'architect but not 'arch', 'orchestra', 'orchid'
                                || StringAt(workingString, (current - 2), "ORCHES", "ARCHIT", "ORCHID") || StringAt(workingString, (current + 2), "T", "S") || ((StringAt(workingString, (current - 1), "A", "O", "U", "E") || (current == 0)) //e.g., 'wachtler', 'wechsler', but not 'tichner'
                    ...
                    case 'G':
                        ...
                        if ((current == 0) && ((workingString[current + 1] == 'Y') || StringAt(workingString, (current + 1), "ES", "EP", "EB", "EL", "EY", "IB", "IL", "IN", "IE", "EI", "ER")))
                        {
                        ...
                        // -ger-,  -gy-
                        if ((StringAt(workingString, (current + 1), "ER") || (workingString[current + 1] == 'Y')) && !StringAt(workingString, 0, "DANGER", "RANGER", "MANGER") && !StringAt(workingString, (current - 1), "E", "I") && !StringAt(workingString, (current - 1), "RGY", "OGY"))
                        {
                            metaphoneData.Add("K", "J");
                            current += 2;
                            break;
                    ...
                    case 'L':
                        if (workingString[current + 1] == 'L')
                        {
                            ...
                            if (((current == (text.Length - 3)) && StringAt(workingString, (current - 1), "ILLO", "ILLA", "ALLE")) || ((StringAt(workingString, (last - 1), "AS", "OS") || StringAt(workingString, last, "A", "O")) && StringAt(workingString, (current - 1), "ALLE")))
                            {
                                metaphoneData.Add("L", " ");
                                current += 2;
                                break;
                    ...
                    case 'W':
                        ...
                        if (((current == last) && IsVowel(workingString[current - 1])) || StringAt(workingString, (current - 1), "EWSKI", "EWSKY", "OWSKI", "OWSKY") || StringAt(workingString, 0, "SCH"))
                        {
                            metaphoneData.Add("", "F");
                            current += 1;
                            break;
        ...
        private static bool StartsWith(this string self, System.StringComparison comparison, params string[] strings)
        {
        ...
        private static bool StringAt(this string self, int startIndex, params string[] strings)
        {
            ...
            return strings.Any(str => self.IndexOf(str, startIndex, System.StringComparison.OrdinalIgnoreCase) >= startIndex);
        }
